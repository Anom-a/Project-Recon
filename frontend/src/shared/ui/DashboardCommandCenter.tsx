...
import { SignalSkeleton } from './LoadingSkeleton';
...
export interface DashboardSignal {
  ...
  tone?: 'red' | 'amber' | 'emerald' | 'blue' | 'purple' | 'slate';
}
...
export default function DashboardCommandCenter({ title, subtitle, signals, loading = false }: DashboardCommandCenterProps) {
  const riskCount = signals.filter(s => s.tone === 'red' || s.tone === 'amber').length;

  return (
    <section className="surface-card mb-5 overflow-hidden rounded-card p-3 sm:p-4" aria-busy={loading}>
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            ...
            <div className="min-w-0">
              <h2 className="truncate text-sm font-black uppercase tracking-wide text-slate-950">{title}</h2>
              <p className="truncate text-xs font-medium text-slate-500">{subtitle}</p>
            </div>
          </div>
        </div>

        <div className="flex items-center gap-2 rounded-xl border border-slate-200/80 bg-slate-50/90 px-3 py-2">
          {loading ? (
            <>
              <Clock className="h-4 w-4 text-slate-400 animate-pulse" aria-hidden />
              <span className="text-xs font-bold text-slate-500">Syncing data…</span>
            </>
          ) : (
            <>
              {riskCount > 0 ? <AlertTriangle className="h-4 w-4 text-amber-500" aria-hidden /> : <CheckCircle2 className="h-4 w-4 text-emerald-500" aria-hidden />}
          ...
          {signals.length === 0 ? (
            ...
            return (
              <motion.div
                ...
                className="interactive-lift rounded-xl border border-slate-200/70 bg-white p-3"
              >
