...
function EnrollmentsPanel() {
  ...
  const [selected, setSelected] = useState<typeof enrollments[0] | null>(null);
  ...
  const updateStatus = (id: string, status: 'active' | 'cancelled') => {
                  ...
                  <td className="px-4 py-3 text-center">
                    <div className="flex items-center justify-center gap-1">
                      ...
                      {e.status === 'pending_payment' && (
                        <button onClick={() => updateStatus(e.id, 'active')} className="p-1 rounded-lg text-slate-400 hover:text-emerald-500 hover:bg-emerald-50" title="Confirm enrollment"><Check className="w-3.5 h-3.5" /></button>
                      )}
                      {(e.status === 'active' || e.status === 'pending_payment') && (
                        <button onClick={() => updateStatus(e.id, 'cancelled')} className="p-1 rounded-lg text-slate-400 hover:text-red-500 hover:bg-red-50" title="Cancel enrollment"><X className="w-3.5 h-3.5" /></button>
                      )}
                    </div>
                  </td>
                </tr>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/30" onClick={() => setSelected(null)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 relative" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-bold text-slate-900">{selected.student}</h3>
              <button onClick={() => setSelected(null)} className="p-1 rounded-lg hover:bg-slate-100"><X className="w-4 h-4" /></button>
            </div>
            <div className="flex flex-col gap-2 text-sm">
              <div className="flex justify-between"><span className="text-slate-500">Class</span><span className="font-medium">{selected.class}</span></div>
              <div className="flex justify-between"><span className="text-slate-500">Date</span><span>{selected.date}</span></div>
              <div className="flex justify-between"><span className="text-slate-500">Fee</span><span className="font-bold">{selected.fee}</span></div>
