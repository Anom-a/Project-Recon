  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setShowIssue(true)} className="flex items-center gap-1.5 bg-brand-red text-white text-xs font-bold px-3 py-2 rounded-lg hover:bg-brand-red-dark transition-colors">
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: 'Total Issued', value: certs.length, icon: Award, color: 'text-brand-red', bg: 'bg-brand-red/5' },
                  ...
                  <td className="px-4 py-3 text-center">
                    {c.pdf ? (
                      <a href={c.pdf} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()}
                        className="inline-flex items-center gap-1 text-[10px] font-bold text-white bg-brand-red px-2.5 py-1.5 rounded-lg hover:bg-brand-red-dark transition-colors">
                ...
                <div className="p-4 space-y-4">
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input type="text" placeholder="Search student by name or email..." value={studentSearch}
                        ...
                        className="w-full pl-9 pr-3 py-2 bg-slate-50 border border-brand-border rounded-lg text-sm focus:outline-none focus:border-brand-red"
                      />
                        ...
                        {studentResults.map(s => {
                          ...
                          return (
                            <button key={s.id} type="button"
                              ...
                              className={`w-full text-left px-3 py-2 text-xs hover:bg-slate-50 flex items-center justify-between ${form.student === s.id ? 'bg-brand-red/5 text-brand-red font-semibold' : 'text-slate-700'}`}
                            >
                              <span>{name}</span>
                  ...
                  <div>
                    ...
                    <select value={form.certificate} onChange={e => setForm(p => ({ ...p, certificate: e.target.value }))}
                      className="w-full px-3 py-2 bg-slate-50 border border-brand-border rounded-lg text-sm focus:outline-none focus:border-brand-red focus:ring-2 focus:ring-brand-red/10"
                    >
